...
class TestSearchProduct(TestCase):
    ...
    def test_subcategory(self):
        ...
        response = self.client.post(reverse('search_products'), body,'json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["data"]["products"]), 0)
    @tag('search','search/products')
    def test_brand(self):
        body = {"brand": [2]}
        response = self.client.post(reverse('search_products'), body,'json')
        ...
        self.assertEqual(response.data["data"]["products"][0]["id"], 2)
    @tag('search','search/products')
    def test_sort(self):
...
class TestSearchBrand(TestCase):
    def setUp(self):
        ...
        Product.objects.create(id=2, name = "Mor T-shirt", price = 100, 
            creation_date = "2019-08-20T07:22:34Z",total_rating = 22, discount=0.1,
            rating_count = 5, stock_amount = 10, short_description = "kışa özel",long_description = "sıcak tutar", subcategory = s, brand = b, vendor = v)
    @tag('search','search/brands')
    def test_no_filter(self):
        body = {"page_size": 5}
        response = self.client.post(reverse('search_brands'), body ,'json')
        #print("RESPONSE")
        #print(response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_subcategory(self):
        ...
        response = self.client.post(reverse('search_brands'), body,'json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["data"]["brands"]), 0)
class TestSearchVendor(TestCase):
    def setUp(self):
        self.client = APIClient()
        c = Category.objects.create(id=4, name="Clothing")
        s = Subcategory.objects.create(id=10, category = c, name="Men's Fashion")
        ...
        Product.objects.create(id=2, name = "Mor T-shirt", price = 100, 
            creation_date = "2019-08-20T07:22:34Z",total_rating = 22, discount=0.1,
            rating_count = 5, stock_amount = 10, short_description = "kışa özel",long_description = "sıcak tutar", subcategory = s, brand = b, vendor = v)
    @tag('search','search/vendors')
    def test_no_filter(self):
        body = {"page_size": 5}
        response = self.client.post(reverse('search_vendors'), body ,'json')
        #print("RESPONSE")
        #print(response.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["data"]["vendors"]), 2)
    @tag('search','search/vendors')
    def test_subcategory(self):
        body = {"subcategory": 2}
        response = self.client.post(reverse('search_vendors'), body,'json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["data"]["vendors"]), 0)
    @tag('search','search/vendors')
    def test_brand(self):
        body = {"brand": [2]}
        response = self.client.post(reverse('search_vendors'), body,'json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["data"]["vendors"]), 1)
        self.assertEqual(response.data["data"]["vendors"][0]["id"], 2)