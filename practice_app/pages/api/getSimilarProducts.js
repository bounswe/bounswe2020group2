const getSimilarProducts = (req, res) =>
  new Promise(async (resolve) => {
    ...
    const [response] = await imageAnnotatorClient
      .batchAnnotateImages({
        requests: [request],
      })
      .catch((err) => {
    ...
    results.forEach((result) => {
      temp +=
        "<h2>Product id:" + result["product"].name.split("/").pop(-1) + "</h2>";
      temp += "<li>Product name: " + result["product"].displayName + "</li>";
      temp +=
        "<li>Product description: " + result["product"].description + "</li>";
      temp +=
        "<li>Product category: " + result["product"].productCategory + "</li>";
