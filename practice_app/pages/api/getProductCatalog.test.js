  ...
  switch (method) {
    case "POST": {
      ...
      // Creates corresponding image of the product
        ...
        client.createReferenceImage(request).then((responses) => {
          // Succesful response
          res.status = 200;
...
describe("It tests the create product set function.", () => {
  ...
  it("It should create new product set", async () => {
    await expect(getProductSet(req)).resolves.toEqual({
      status: 200,
      statusText: "Product set is created.",
    });
  });
...
describe("It tests the update product function.", () => {
  const req = {
    method: "PATCH",
    body: {
      id: "1",
  ...
  it("It should update the style label of the product", async () => {
    await expect(getProductSet(req)).resolves.toEqual({
      status: 200,
      statusText:
        "projects/brilliant-era-276800/locations/europe-west1/products/5 is updated.",
    });
  });
});

describe("It tests the delete product function.", () => {
  const req = {
    method: "DELETE",
    body: {
      id: "5",
  ...
  it("It should delete the product", async () => {
    await expect(getProductSet(req)).resolves.toEqual({
      status: 200,
      statusText: "The product is deleted.",
    });
  });

  it("It should not delete the product that does not exist.", async () => {
    await expect(getProductSet(req)).rejects;
  });
});
