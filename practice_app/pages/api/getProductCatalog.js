...
async function createProduct(req, res) {
    ...
    // Creates product set
        ...
        const [createdProductSet] = await client.createProductSet(request).catch(err => {
            res.json({
                statusCode: 400,
                statusText: 'Display name is missing, or is longer than 4096 characters.',
            })
        })
        // Initializes both status message and status code of the succesful response
    ...
    // Creates corresponding image of the product
        ...
        client
            ...
            .catch(err => {
                res.json({
                    statusCode: 400,
                    statusText: 'The image uri or the product does not exist or the product has already image.',
                })
            })
    }
}
