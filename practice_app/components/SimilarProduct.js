...
import CSS_COLOR_NAMES from '../cssColors.json'
...
export default function SimilarProduct({}) {
    ...
    const onGetSimilarProducts = async event => {
        ...
        if (
            !['style=women', 'style=men', 'style=children', 'style=home', 'style=general', 'style=packaged'].includes(
                filter,
            )
        ) {
            alert('Invalid style name. Valid ones are: women, men, children, home, general and packaged')
            return
        }
    ...
    const onCreateProduct = async event => {
        ...
        if (
            ...
            alert('please fill product set id, product id, name, category, description, color and style')
            return
        }
        ...
        if (!CSS_COLOR_NAMES.map(v => v.toLowerCase()).includes(inputs.color_title.toLowerCase())) {
            alert('The color name must be one of the CSS color.')
            return
        }

        // checks whether the style given by user is valid
        if (!['women', 'men', 'children', 'home', 'general', 'packaged'].includes(inputs.style_title)) {
            alert('Invalid style name. Valid ones are: women, men, children, home, general and packaged')
            return
        }
    ...
    const onUpdate = async event => {
        const inputs = getFormValues(event)
        // checks whether all the inputs are given
        if (inputs.updated_title === '' || inputs.key_title === '' || inputs.value_title === '') {
            alert('please fill product id, key and key value')
            return
        }
        if (
            inputs.key_title.toLowerCase() == 'style' &&
            !['women', 'men', 'children', 'home', 'general', 'packaged'].includes(inputs.value_title)
        ) {
            alert('Invalid style name. Valid ones are: women, men, children, home, general and packaged')
            return
        }
        if (
            ...
            !CSS_COLOR_NAMES.map(v => v.toLowerCase()).includes(inputs.value_title.toLowerCase())
        ) {
            alert('The color name must be one of the CSS color.')
            return
        }
    ...
    return (
        <div className="container">
            ...
            <h2 className="vendor">Do you want to create your own product set?</h2>
            <form onSubmit={onCreateProductSet}>
                <label>
                    SetID:
                    <input type="text" placeholder="Add set id" name="product_set_title" required />
                </label>
                <label>
                    SetName:
                    <input type="text" placeholder="Add name" name="product_set_display_title" required />
                </label>
                <input type="submit" value="Create" />
            </form>

            <h2 className="vendor">Do you want to add your product into your product set?</h2>
            <form onSubmit={onCreateProduct}>
                <label>
                    ProductID:
                    <input type="text" placeholder="Add product id" name="product_title" required />
                </label>
                <label>
                    SetID:
                    <input type="text" placeholder="Add set id" name="product_set_id_title" required />
                </label>
                <label>
                    ProductName:
                    <input type="text" placeholder="Add name" name="product_display_title" required />
                </label>
                <label>
                    Category:
                    <input type="text" placeholder="Add category" name="category_title" required />
                </label>
                <label>
                    Description:
                    <input type="text" placeholder="Write description" name="description_title" required />
                </label>
                <label>
                    Color:
                    <input type="text" placeholder="Write color" name="color_title" required />
                </label>
                <label>
                    Style:
                    <input type="text" placeholder="Write style" name="style_title" required />
                </label>
                <input type="submit" value="Add" />
            </form>
            <h2>We are curious about how your product looks like! Do you want to add its image?</h2>
            <form onSubmit={onRefImg}>
                <label>
                    ProductID:
                    <input type="text" placeholder="Add product id" name="img_product_title" required />
                </label>
                <label>
                    ReferenceUri:
                    <input type="text" placeholder="Add reference image" name="img_uri_title" required />
                </label>
                <input type="submit" value="Add" />
            </form>
            ...
            <form onSubmit={onUpdate}>
                <label>
                    ProductID:
                    <input type="text" placeholder="Add product id" name="updated_title" required />
                </label>
                <label>
                    Key:
                ...
                <label>
                    KeyValue:
                    <input type="text" placeholder="Add value" name="value_title" required />
                </label>
            ...
            <form onSubmit={onDeleteProduct}>
                <label>
                    ProductID:
                    <input type="text" placeholder="Add product id" name="delete_title" required />
                </label>
            ...
            <form onSubmit={onDeleteProductSet}>
                <label>
                    ProductSetID:
                    <input type="text" placeholder="Add product set id" name="delete_set_title" required />
                </label>
                <input type="submit" value="Delete" />
            </form>
            <h1>Are you customer?</h1>
            <h2>Do you want to find out which brands are in Getflix? Let's list all product sets!</h2>
            <form target="_blank" action="/api/getSets" method="GET">
                <input type="submit" value="List" />
            </form>
            <h2>Hmm! There are lots of products in our website! Take a look all of them!</h2>
            <form target="_blank" action="/api/getAllProducts" method="GET">
                <input type="submit" value="List" />
            </form>
            <h2>Are you interested only in products of your favorite vendor? Really?? </h2>
            ...
            <form target="_blank" action="/api/getProductsByCategory" method="GET">
                ...
                <label>
                    Category:
                    <input type="text" placeholder="Add category name" name="category" required />
                </label>
                <input type="submit" value="Filter" />
            </form>
            <h2 className="special">
                Let's play a game! You give us an image of a product, we give our products sorted by similarity
                percentages!
            </h2>
            <form onSubmit={onGetSimilarProducts}>
                <label>
                    ProductSetID: <input type="text" placeholder="Add product set id" name="setId" required />
                </label>
                <label>
                    Category: <input type="text" placeholder="Add category" name="category" required />
                </label>
                <label>
                    File: <input type="file" placeholder="Add path of file" name="file" required />
                </label>
                <label>
                    Filter: <input type="text" placeholder="Add filter" name="filter" required />
                </label>
                <input type="submit" value="List" />
            </form>
            {similarProducts &&
